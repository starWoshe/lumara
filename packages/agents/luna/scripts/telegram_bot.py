...
Опційні змінні середовища:
  ...
  META_USER_TOKEN            — 60-денний токен Meta (для FB/IG/Threads)
  LUNA_PAGE_ID               — Facebook Page ID LUNA
  LUNA_IG_USER_ID            — Instagram Business Account ID LUNA
  LUNA_THREADS_USER_ID       — Threads User ID LUNA (опційно)
...
def main():
    ...
    meta_token   = os.environ.get('META_USER_TOKEN', '').strip()
    ...
    print(f'  Meta (FB/IG): {"✅" if meta_token else "⏭️  META_USER_TOKEN не вказано"}')
    print()
